...
| `NANO_PR_AUTO_MERGE` | `1` | after convergence, run the merge stage; `0` = stop at `converged` (review-only). Per-submit override via the positive `autoMerge` field on `start/convergence-loop`; legacy `convergeOnly: true` remains accepted when `autoMerge` is omitted |
