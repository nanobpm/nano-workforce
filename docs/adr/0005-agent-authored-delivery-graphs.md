...
> carry guards is an **exclusive** (data-based) split instead of the default parallel fan-out; its
> guarded branches compile to a BPMN `exclusiveGateway` (`gwx<i>`) with one FEEL
