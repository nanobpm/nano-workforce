...
function memApp(): {
    ...
    const app = testBoundary<AppApi>(testBoundary({ data: { table: (n: string, pk?: string) => tbl(n, pk) } }));
    return { app, stores };
}
...
async function call(app: AppApi, method: string, query: Record<string, string>, body?: unknown) {
    const res = await handler({ req: testBoundary(req(method, query)), body }, app);
...
Deno.test("GET ?since returns only newer entries", async () => {
    ...
    assertEquals(tail.body.entries?.map((e) => String(e.body)), ["two"]);
});
