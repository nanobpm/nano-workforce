...
function datasourcePath(url: string): string | null {
	...
	const decode = (p: string): string => {
		try {
			...
			throw new Error(`could not decode datasource path in URL: ${url}`, {
				cause: e,
			});
		}
	};
...
function listFiles(root: string, skipTop: Set<string> = new Set()): string[] {
	...
	const walk = (dir: string): void => {
		for (const entry of readdirSync(dir, { withFileTypes: true })) {
			if (dir === root && entry.isDirectory() && skipTop.has(entry.name))
				continue;
...
function extractTarball(tgz: string, tmp: string): string {
	...
	for (const raw of listing.split("\n")) {
		...
		if (isAbsolute(entry) || /^[A-Za-z]:[\\/]/.test(entry)) {
			throw new Error(
				`refusing to extract tarball with absolute path entry: ${entry} (${tgz})`,
			);
		}
		const parts = entry.split(/[\\/]/);
		if (parts.includes("..")) {
			throw new Error(
				`refusing to extract tarball with '..' path segment: ${entry} (${tgz})`,
			);
		}
	}
	...
	if (!existsSync(pkgDir))
		throw new Error(`extracted tarball has no package/ dir: ${tgz}`);
	return pkgDir;
}
...
function runTar(tarArgs: string[], tgz: string): string {
	try {
		...
		if ((e as NodeJS.ErrnoException).code === "ENOENT") {
			throw new Error(
				`'tar' not found on PATH — install it to extract ${tgz}`,
				{ cause: e },
			);
		}
		// biome-ignore lint/plugin: runtime/framework contract boundary for external data shape
		throw new Error(`tar failed on ${tgz}: ${(e as Error).message}`, {
			cause: e,
		});
	}
}
...
function main(): void {
	...
	try {
		...
		console.log(
			`• source ${srcLabel} @ ${srcVersion}  →  current @ ${curVersion}\n`,
		);
		...
		if (skipped.length)
			console.log(`  preserved (not overlaid): ${skipped.length} file(s)`);
...
function report(label: string, files: string[]): void {
	...
	if (files.length > show.length)
		console.log(`    … and ${files.length - show.length} more`);
}
