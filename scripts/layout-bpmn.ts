...
async function defaultProcessFiles(): Promise<string[]> {
	...
	if (g.Deno?.readDir) {
		const files: string[] = [];
		for await (const e of g.Deno.readDir(dir))
			if (e.isFile && e.name.endsWith(".bpmn")) files.push(`${dir}/${e.name}`);
		return files.sort();
	}
...
async function main() {
	...
	if (files.length === 0) {
		console.error(
			"usage: layout-bpmn [--check] [file.bpmn ...] (default: resources/processes/*.bpmn)",
		);
		exit(2);
	}
	const stale: string[] = [];
	for (const file of files) {
		...
		if (check) {
			if (laid !== current) {
				stale.push(file);
				console.error(
					`[layout:check] ${file}: DI is STALE — re-run \`npm run layout ${file}\` and commit`,
				);
			} else {
				console.log(
					`[layout:check] ${file}: DI fresh (${shapes} shapes + ${edges} edges)`,
				);
			}
			continue;
		}
