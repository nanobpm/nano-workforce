...
Deno.test("every page datasource table exists in the migrations", async () => {
	...
	assert(
		refs.length > 0,
		"no datasource references found — collector or pages are broken",
	);
	for (const r of refs) {
...
Deno.test("issue #87: plan_reviews is surfaced on the epic and home pages", async () => {
	const refs = await loadRefs();
	const onEpic = refs.some(
		(r) => r.page === "epic.page.json" && r.table === "plan_reviews",
	);
	const onHome = refs.some(
		(r) => r.page === "home.page.json" && r.table === "plan_reviews",
	);
	assert(
		onEpic,
		"epic.page.json must bind a grid to plan_reviews (plan-review trace)",
	);
	assert(
		onHome,
		"home.page.json plan detail must include a plan_reviews child grid",
	);
