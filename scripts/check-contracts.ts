...
const CONFIG_KEY = /\bprocess\.env\.([A-Z][A-Z0-9_]*)\b/g;
const CONFIG_FAMILY = /^(NANO_|NANOBPMN_|CAMUNDA_|PR_REVIEW_)/;
const EXPLICIT_CONFIG_KEYS = new Set(["PR_REVIEW_PORT"]);

// The registry module declares the keys and (in comments/strings) names the rejected synonyms, and
// this checker's own doc comment shows the `process.env.NANO_*` patterns it scans for — both would
// self-trip the scan, so they are exempt.
const EXEMPT_FILES = new Set(["app/contracts.ts", "scripts/check-contracts.ts"]);
...
function main(): void {
  ...
  for (const file of collectFiles()) {
    ...
    for (const m of src.matchAll(CONFIG_KEY)) {
      const key = m[1];
      if (rejected.has(key)) {
        errors.push(
