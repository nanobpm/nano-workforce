...
test("complete-user-task: completes a plan-review escalation and drops its read-model row", async () => {
  ...
  assertEquals(completed, [{ userTaskKey: "ut-1", variables: { directive: "revise", notes: "narrow scope", completedUserTaskKey: "ut-1" } }]);
...
test("complete-user-task: completes a trial-merge escalation with the typed action variable", async () => {
  ...
  assertEquals(res.status, 200);
  assertEquals(completed, [{ userTaskKey: "ut-2", variables: { action: "rebase", completedUserTaskKey: "ut-2" } }]);
});
...
test("complete-user-task: completes a feature-blocked acknowledgement with the typed note (issue #332)", async () => {
  ...
  assertEquals(completed, [{ userTaskKey: "ut-4", variables: { note: "reassigned", completedUserTaskKey: "ut-4" } }]);
});
...
test("complete-user-task: completes a feature-escalation answer (issue #332)", async () => {
  ...
  assertEquals(completed, [{ userTaskKey: "ut-6", variables: { resolution: "answer", answer: "use v2", completedUserTaskKey: "ut-6" } }]);
});
...
test("complete-user-task: a read-model cleanup failure does not mask a resumed completion (200)", async () => {
  ...
  assertEquals(res.status, 200);
  assertEquals(res.body.ok, true);
  assertEquals(completed, [{ userTaskKey: "ut-5", variables: { directive: "revise", notes: "narrow scope", completedUserTaskKey: "ut-5" } }]);
});
