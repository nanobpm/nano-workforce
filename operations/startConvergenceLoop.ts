...
export default defineOperation<
	...
	| { prKey: string; alreadyRunning?: boolean; processKey?: string | null }
	| { error: string }
>("startConvergenceLoop", async ({ body }, app) => {
	...
	if (!parsed) {
		return {
			status: 400,
			body: { error: "could not parse PR (use owner/repo#123 or a PR URL)" },
		};
	}
	const dependsOn = Array.isArray(vars.dependsOn)
		? vars.dependsOn.map((d) => String(d))
		: [];
