...
function resolveGitDirs(): { gitDir: string; commonDir: string } | null {
	...
	const commonDir = common?.trim()
		? isAbsolute(common.trim())
			? common.trim()
			: resolve(gitDir, common.trim())
		: gitDir;
...
function urbanVersion(): string | null {
	const pkg = readJson(
		join(REPO_ROOT, "node_modules", "@nanobpm", "urban", "package.json"),
	);
	return typeof pkg?.version === "string" ? pkg.version : null;
}
...
function gitSha(): string | null {
	...
	const loose =
		readText(join(dirs.gitDir, refPath)) ??
		readText(join(dirs.commonDir, refPath));
...
function gitBranch(): string | null {
	...
	return refPath.startsWith("refs/heads/")
