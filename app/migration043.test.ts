// Regression guard for migration 043 (issue #292 slice S2): the durable constraints on the FK-FREE
...
function migratedDb(): DatabaseSync {
  ...
  const sql = readFileSync(
    fileURLToPath(new URL("../db/migrations/043_epic_set_admission_staging.sql", import.meta.url)),
    "utf8",
  );
  db.exec(sql);
  return db;
...
test("migration 043 applies cleanly with NO plans table (FK-free) and stages an edge", () => {
  const db = migratedDb();
...
test("migration 043 stages an admitted epic with no plans row", () => {
  const db = migratedDb();
  insertEpic(db, "o/r#1");
...
test("migration 043 CHECK rejects a self-edge", () => {
  const db = migratedDb();
  let threw = false;
  try {
...
test("migration 043 PRIMARY KEY rejects a duplicate consumer→producer edge", () => {
  const db = migratedDb();
...
test("migration 043 PRIMARY KEY rejects a duplicate staged epic", () => {
  const db = migratedDb();
  insertEpic(db, "o/r#1");
  let threw = false;
  try {
