...
Deno.test("recordTaskDelta: upserts per (plan, task) — a resume overwrites, not duplicates", async () => {
	...
	const first = await recordTaskDelta(
		data,
		"o/r#1",
		"gap-2",
		{
	...
	const second = await recordTaskDelta(
		data,
		"o/r#1",
		"gap-2",
		{
	...
	assertEquals(
		entry.contractChange,
		undefined,
		"cleared when the new report omits it",
	);
...
Deno.test("readTaskDeltas: scoped to a plan, in write order, arrays decoded", async () => {
	const { data } = memData();
	await recordTaskDelta(data, "o/r#1", "gap-2", {
		newlyTouches: ["a.rs"],
		affectsTasks: [],
	});
	await recordTaskDelta(data, "o/r#1", "gap-8", {
		newlyTouches: [],
		affectsTasks: ["gap-2"],
		constraint: "x",
	});
	await recordTaskDelta(data, "o/r#2", "gap-1", {
		newlyTouches: ["z.rs"],
		affectsTasks: [],
	}); // other plan
...
Deno.test("aggregateEpicDeltas: unions touched files + affected tasks, lists changes/constraints", async () => {
	...
	assertEquals(
		report.touchedFiles,
		["engine/state.rs", "engine/tests.rs"],
		"union, sorted, de-duplicated",
	);
	assertEquals(report.affectedTasks, ["gap-2", "gap-4", "gap-8"]);
	assertEquals(report.contractChanges, [
		{ taskId: "gap-2", change: "restructured complete_adhoc_tool" },
	]);
	assertEquals(report.constraints, [
		{ taskId: "gap-8", constraint: "results:[] seed" },
	]);
...
Deno.test("clearTaskDeltas: drops a plan's whole set (re-plan cleanup), leaving other plans intact", async () => {
	...
	await recordTaskDelta(data, "other", "gap-1", {
		newlyTouches: ["z.rs"],
		affectsTasks: [],
	});
	...
	assertEquals(
		stores["plan_task_deltas"].length,
		1,
		"the other plan's delta survives",
	);
});
