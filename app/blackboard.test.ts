...
import { appendEntry, blackboardUrl, detectFileClaimConflicts, isUniqueViolation, mintBlackboardToken, normalizeKind, planKeyForToken, publicBaseUrl, readBlackboard, readBlackboardPage, renderCoordinationBrief, } from "./blackboard.ts";
...
Deno.test("publicBaseUrl: a blank/whitespace override falls back instead of yielding a bad URL", () => {
    ...
    try {
        ...
        if (prev === undefined)
            ...
            process.env.NANO_PR_BASE_URL = prev;
    }
});
Deno.test("blackboardUrl: capability token rides the query string", () => {
    assertEquals(blackboardUrl("tok+en/x", "https://h"), "https://h/hooks/blackboard?token=tok%2Ben%2Fx");
});
...
Deno.test("appendEntry: a missing author defaults to 'system' and kind is normalised", async () => {
    const { data } = memData();
    await appendEntry(data, "p", { body: "x", kind: testBoundary<unknown>("weird") });
    const [e] = await readBlackboard(data, "p");
...
Deno.test("detectFileClaimConflicts: your own prior claim and non-file-claim entries are not conflicts", async () => {
    ...
    assertEquals(await detectFileClaimConflicts(data, "p", { author_task: "gap-2", files: ["a.rs"] }), []);
