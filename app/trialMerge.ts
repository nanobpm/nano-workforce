...
export function trialMergeDecision(
	result: TrialMergeResult,
): TrialMergeDecision {
	return result === "suite-failed" ? "escalate" : "proceed";
}

export function shouldRunTrialMerge(
	headCount: number,
	protocol: Pick<MergeProtocol, "land">,
): boolean {
...
const auditTable = (data: DataLayer) =>
	data.table<TrialMergeAuditRow>("plan_trial_merges", "id");
...
export async function recordTrialMergeAudit(
	...
	if (jobKey) {
		const existing = (
			await table.find({ plan_key: row.planKey, job_key: jobKey })
