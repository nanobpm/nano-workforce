...
function stubFetch(pages: number[]) {
	...
	return (url: string | URL | Request): Promise<Response> => {
		...
		if (page < pages.length) {
			headers.set(
				"link",
				`<https://api.github.com/next?page=${page + 1}>; rel="next"`,
			);
		}
...
async function withTokenTransport<T>(
	pages: number[],
	fn: () => Promise<T>,
): Promise<T> {
...
Deno.test("fetchPrFiles: returns the complete list for a sub-cap PR (short final page)", async () => {
	const files = await withTokenTransport([100, 42], () =>
		fetchPrFiles("o/r", 1, "tok"),
	);
...
Deno.test("fetchPrFiles: exactly 500 files with no next page is complete, not truncated", async () => {
	...
	const files = await withTokenTransport([100, 100, 100, 100, 100], () =>
		fetchPrFiles("o/r", 2, "tok"),
	);
...
Deno.test("fetchPrFiles: throws when the cap genuinely truncates (full last page + next)", async () => {
	...
	await assertRejects(
		() =>
			withTokenTransport([100, 100, 100, 100, 100, 100], () =>
				fetchPrFiles("o/r", 3, "tok"),
			),
		Error,
		"truncated",
	);
