...
function memData(opts: {
    ...
    let insertCalls = 0;
    function tbl(_name: string, key: string) {
        return {
    ...
    const data = testBoundary<DataLayer>(testBoundary({ table: (n: string, k: string) => tbl(n, k) }));
...
Deno.test("ensurePr reconstructs a minimal converging row when the parent is absent", async () => {
    ...
    const row = testBoundary<Record<string, unknown>>(rows.get("o/r#2"));
    ...
    assert(typeof row.abandon_token === "string" && (testBoundary<string>(row.abandon_token)).length > 0);
});
Deno.test("ensurePr defaults current_round to 1 (rounds are 1-based) when none is passed", async () => {
    ...
    assertEquals(testBoundary<Record<string, unknown>>(rows.get("o/r#5")).current_round, 1, "an unknown round heals to 1, not 0, matching submitPr's 1-based invariant");
});
Deno.test("ensurePr reuses a supplied abandon token instead of minting a new one", async () => {
    ...
    assertEquals(testBoundary<Record<string, unknown>>(rows.get("o/r#7")).abandon_token, "TOK-en_123", "the running agent's existing token is preserved so its abort check keeps resolving");
});
Deno.test("ensurePr mints a token when none is supplied", async () => {
    const { data, rows } = memData();
    await ensurePr(data, { prKey: "o/r#8", repo: "o/r", number: 8 });
    const tok = testBoundary<Record<string, unknown>>(rows.get("o/r#8")).abandon_token;
    assert(typeof tok === "string" && (testBoundary<string>(tok)).length > 0, "a fresh token is minted as a fallback");
});
Deno.test("ensurePr prefers an explicit url over the canonical one", async () => {
    ...
    assertEquals(testBoundary<Record<string, unknown>>(rows.get("o/r#9")).url, url);
});
