...
import { autoRetroEnabled, gatherRetro, isDigestEmpty, isPlanComplete, maybeStartRetro, planKeyForPr, recordRetro, renderRetroBrief, } from "./retro.ts";
...
function fakeEngine(): {
    ...
    const engine = testBoundary<EngineClient>(testBoundary({
        async createInstance(req: Record<string, unknown>) {
...
function seedPlan(stores: Record<string, Record<string, unknown>[]>, over: Record<string, unknown> = {}) {
    stores.plans = [{
            plan_key: PLAN,
            repo: "acme/widgets",
...
function seedTask(stores: Record<string, Record<string, unknown>[]>, task: Record<string, unknown>) {
    const tasks = stores.plan_tasks ?? [];
    stores.plan_tasks = tasks;
    tasks.push({ plan_key: PLAN, ...task });
}
function seedPr(stores: Record<string, Record<string, unknown>[]>, pr_key: string, status: string) {
    const prs = stores.pull_requests ?? [];
    stores.pull_requests = prs;
    prs.push({ pr_key, status });
}
function seedReview(stores: Record<string, Record<string, unknown>[]>, round: number, approved: number, findings: string | null) {
    const reviews = stores.plan_reviews ?? [];
    stores.plan_reviews = reviews;
    reviews.push({ plan_key: PLAN, round, approved, findings, created_at: `t${round}`, job_key: null });
}
Deno.test("autoRetroEnabled: on by default; disabled by 0/false/off/no", () => {
    ...
    try {
        ...
        if (prev == null)
            ...
            process.env.NANO_AUTO_RETRO = prev;
    }
});
...
Deno.test("recordRetro: rethrows a non-unique (FOREIGN KEY) constraint error instead of swallowing it", async () => {
    ...
    const data = testBoundary<DataLayer>(testBoundary({ table: () => table }));
    let threw = false;
    try {
...
Deno.test("maybeStartRetro: a createInstance failure records a blocked retro (fire-once guard already consumed)", async () => {
    ...
    seedPr(stores, "acme/widgets#10", "merged");
    await appendEntry(data, PLAN, { author_task: "t1", kind: "learning", body: "regen first" });
    const engine = testBoundary<EngineClient>(testBoundary({ async createInstance() { throw new Error("gateway down"); } }));
...
Deno.test("maybeStartRetro: a secondary blocked-retro persistence failure still returns start-failed (not error)", async () => {
    ...
    const failingData = testBoundary<DataLayer>(testBoundary({
        table: (name: string, pk?: string) => {
            const t = (testBoundary(data)).table(name, pk);
            if (name !== "plan_retros")
                return t;
