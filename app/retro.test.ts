...
Deno.test("gatherRetro: separates learnings from notes and folds in deltas", async () => {
	const { data, stores } = memData();
	seedPlan(stores);
	await appendEntry(data, PLAN, {
		author_task: "t1",
		kind: "learning",
		body: "regen the API surface before building",
	});
	await appendEntry(data, PLAN, {
		author_task: "t2",
		kind: "learning",
		body: "use nextest not cargo test",
	});
	await appendEntry(data, PLAN, {
		author_task: "t3",
		kind: "note",
...
Deno.test("gatherRetro: folds in the plan-review trace and task-outcome shape", async () => {
	...
	assertEquals(
		d.reviewRejections.map((r) => r.round),
		[0, 1],
	);
	assertEquals(
		d.reviewRejections[0].findings,
		"task t2 depends on t3 but is sequenced first",
	);
...
Deno.test("renderRetroBrief: renders learnings + constraints; states 'none' with no learnings", () => {
	...
	const brief = renderRetroBrief({
		...
		notes: [
			{ author_task: "t2", kind: "note", body: "watch the release lane" },
		],
		reviewRounds: 2,
...
Deno.test("isDigestEmpty: empty only with no learnings/deltas/notes AND no review rejections", () => {
	...
	assert(
		isDigestEmpty({ ...base, counts: { learnings: 0, deltas: 0, notes: 0 } }),
	);
	assert(
		!isDigestEmpty({ ...base, counts: { learnings: 1, deltas: 0, notes: 0 } }),
	);
	assert(
		!isDigestEmpty({ ...base, counts: { learnings: 0, deltas: 2, notes: 0 } }),
	);
	assert(
		!isDigestEmpty({ ...base, counts: { learnings: 0, deltas: 0, notes: 1 } }),
	);
	...
	assert(
		!isDigestEmpty({
			...base,
			reviewRounds: 2,
			reviewRejections: [{ round: 0, findings: "bad seq" }],
			counts: { learnings: 0, deltas: 0, notes: 0 },
		}),
	);
	...
	assert(
		isDigestEmpty({
			...
			reviewRounds: 1,
			planApproved: true,
			taskOutcomes: { total: 3, byStatus: { opened: 3 } },
			counts: { learnings: 0, deltas: 0, notes: 0 },
		}),
	);
});
...
Deno.test("recordRetro: inserts then updates the same plan_key row in place", async () => {
	const { data, stores } = memData();
	await recordRetro(data, PLAN, {
		status: "filed",
		prKey: "acme/widgets#20",
		learnings: 3,
		summary: "promoted 2",
	});
	...
	assertEquals(
		stores["plan_retros"].length,
		1,
		"same plan_key must not duplicate",
	);
...
Deno.test("recordRetro: rethrows a non-unique (FOREIGN KEY) constraint error instead of swallowing it", async () => {
	...
	assertEquals(
		updated,
		false,
		"must not silently fall back to update on a non-unique error",
	);
});
...
Deno.test("maybeStartRetro: starts the retro exactly once when the last PR lands with material", async () => {
	...
	assert(
		stores["plans"][0].retro_started_at,
		"retro_started_at must be stamped",
	);
...
Deno.test("maybeStartRetro: a createInstance failure records a blocked retro (fire-once guard already consumed)", async () => {
	...
	assertStringIncludes(
		String(stores["plan_retros"][0].summary),
		"gateway down",
	);
});
...
Deno.test("maybeStartRetro: bails while the plan is incomplete", async () => {
	...
	assertEquals(
		stores["plans"][0].retro_started_at,
		null,
		"must not stamp an incomplete plan",
	);
});
...
Deno.test("maybeStartRetro: complete but empty → records a skipped retro, does not start the process", async () => {
	...
	assert(
		stores["plans"][0].retro_started_at,
		"stamped so we don't re-check forever",
	);
	assertEquals(stores["plan_retros"][0].status, "skipped");
});
