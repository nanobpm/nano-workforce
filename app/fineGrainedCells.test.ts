...
// escalation through the shared `human-escalation` cell) plus the first live composition seam:
// `feature.bpmn` now hands its converge step to the `converge-cell` via `callActivity`, so the
// inlined `pr.converge-feature` serviceTask no longer sits on the feature path. The behavioural
// contracts these cells reuse (senior:feature implement loop, trial-merge, readiness probe) are
// already exercised by the existing suites; this guard pins the decomposition so the shared cells
// can't silently drift back into per-caller copies.
