...
const retroStartsTbl = (data: DataLayer) =>
	data.table<{ plan_key: string; started_at: string }>(
		"plan_retro_starts",
		"plan_key",
	);
...
export async function planKeyForPr(
	data: DataLayer,
	prKey: string,
...
export async function isPlanComplete(
	data: DataLayer,
	planKey: string,
): Promise<boolean> {
...
export async function gatherRetro(
	...
	const planApproved =
		reviews.length > 0 && reviews[reviews.length - 1].approved === 1;
...
export function renderRetroBrief(d: RetroDigest): string {
	...
	if (d.reviewRejections.length === 0) {
		...
		lines.push(
			`The plan was revised after ${d.reviewRejections.length} rejected round(s):`,
		);
		for (const r of d.reviewRejections)
			lines.push(`- **round ${r.round}**: ${r.findings}`);
	}
	if (d.taskOutcomes.total > 0) {
		...
		lines.push(
			"",
			`### Task outcomes (${d.taskOutcomes.total} task(s))`,
			shape,
		);
	}
	lines.push(
...
const retrosTbl = (data: DataLayer) =>
	data.table<{ plan_key: string } & Record<string, unknown>>(
		"plan_retros",
		"plan_key",
	);

async function claimRetroStart(
	data: DataLayer,
	planKey: string,
): Promise<boolean> {
	try {
...
export async function maybeStartRetro(
	...
	try {
		...
		if (plan.retro_started_at)
			return { started: false, planKey, reason: "already-started" };

		if (!(await isPlanComplete(data, planKey)))
			return { started: false, planKey, reason: "incomplete" };

		const digest = await gatherRetro(data, planKey);
		if (isDigestEmpty(digest)) {
			if (!(await claimRetroStart(data, planKey)))
				return { started: false, planKey, reason: "already-started" };
			...
			await plansTbl(data).update(planKey, {
				retro_started_at: now(),
				updated_at: now(),
			});
			await recordRetro(data, planKey, {
				status: "skipped",
				summary: "No learnings, deltas, or notes to retrospect.",
			});
		...
		try {
			...
			log?.("error", `retro: could not start process for epic ${planKey}`, {
				err: String(err),
			});
			...
			try {
				...
				log?.(
					"error",
					`retro: could not persist blocked retro for epic ${planKey}`,
					{
		...
		log?.("info", `retro: started for epic ${planKey}`, {
			processInstanceKey,
			learnings: digest.counts.learnings,
		});
		return { started: true, planKey };
	} catch (err) {
		log?.("error", `retro: could not start for PR ${prKey}`, {
			err: String(err),
		});
