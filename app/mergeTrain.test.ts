...
Deno.test("planLane: dependency depth then task id picks the single lane head", () => {
	...
	assertEquals(
		planLane(["gap-8", "gap-9", "gap-2"], taskToPr, new Set(), depth),
		{
			headTaskId: "gap-2",
			headPrKey: "o/r#2",
...
Deno.test("planLane: already-merged head advances to the next unmerged member", () => {
	assertEquals(
		planLane(["gap-2", "gap-8", "gap-9"], taskToPr, new Set(["o/r#2"])),
		{
			headTaskId: "gap-8",
			headPrKey: "o/r#8",
			heldTaskIds: ["gap-9"],
...
Deno.test("planPrLane: lane head and PRs outside exclusion lanes are not held", () => {
	assertEquals(
		planPrLane([["gap-2", "gap-8"], ["gap-10"]], taskToPr, new Set(), "o/r#2")
			.isHeld,
		false,
	);
	assertEquals(
		planPrLane([["gap-2", "gap-8"], ["gap-10"]], taskToPr, new Set(), "o/r#10")
			.isHeld,
		false,
	);
	assertEquals(
		planPrLane([["gap-2", "gap-8"]], taskToPr, new Set(), "o/r#404").isHeld,
		false,
	);
});
