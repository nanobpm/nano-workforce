...
Deno.test("conflict routes to the rebase budget gate, not straight to a human", () => {
	...
	assert(
		...
		"gw-mergeable → gw-rebase (conflict) missing",
	);
...
Deno.test("rebase arm mirrors the fix-ci arm: budget gate → agent → result gate", () => {
	...
	assert(
		hasFlow("gw-rebase", "rebase"),
		"gw-rebase → rebase (within budget) missing",
	);
	assert(
		hasFlow("gw-rebase", "merge-esc-conflict"),
		"gw-rebase → merge-esc-conflict (budget exhausted) missing",
	);
	...
	assert(
		hasFlow("rebase", "gw-rebase-result"),
		"rebase → gw-rebase-result missing",
	);
...
Deno.test("rebase result: success re-arms the poller; unresolved escalates to a human", () => {
	...
	assert(
		hasFlow("gw-rebase-result", "arm-merge"),
		"gw-rebase-result → arm-merge (rebased) missing",
	);
...
Deno.test("regression: the conflict verdict passes through the rebase actor, not straight to escalation", () => {
	...
	assert(
		hasFlow("gw-mergeable", "attempt-merge"),
		"ready → attempt-merge path should still exist",
	);
});
