...
export function computeWaves(tasks: readonly WaveTask[]): WaveResult {
	...
	for (const t of tasks) {
		...
		for (const raw of t.dependsOn ?? []) {
			...
			if (!ids.has(d))
				throw new WaveError(`task "${t.id}" depends on unknown task "${d}"`);
			deps.push(d);
		}
	...
	const level = (id: string): number => {
		...
		if (visiting.has(id))
			throw new WaveError(`dependency cycle detected at task "${id}"`);
		visiting.add(id);
		let lvl = 0;
...
export function waveMergeTargets(
	...
	for (const t of tasks) {
		...
		if ((t.status !== "opened" && t.status !== "waiting-for-lane") || !t.pr_key)
			continue;
		keys.push(t.pr_key);
	}
	return keys;
