...
Deno.test("pollIncidents mirrors an ACTIVE incident onto the PR row, then clears it, leaving status untouched", async () => {
	...
	globalThis.fetch = incidentFetch({
		"PI-7": [
			{
				incidentKey: "INC-1",
				errorMessage: "boom: unhandled error",
				state: "ACTIVE",
...
Deno.test("pollIncidents picks the oldest incident by creationTime, sorting a missing timestamp last", async () => {
	...
	globalThis.fetch = incidentFetch({
		"PI-11": [
			{
				incidentKey: "INC-NOTS",
				errorMessage: "no timestamp",
				state: "ACTIVE",
			},
			{
				incidentKey: "INC-OLD",
				errorMessage: "the first fault",
				state: "ACTIVE",
				creationTime: "2024-01-01T00:00:00Z",
			},
			{
				incidentKey: "INC-NEW",
				errorMessage: "a later fault",
				state: "ACTIVE",
...
Deno.test("submitPr stringifies a numeric processInstanceKey (contract: string | null)", async () => {
	await withGithubOff(async () => {
		...
		const engine = {
			...
			createInstance: () =>
				Promise.resolve({ processInstanceKey: 2251799813685249 }),
			// deno-lint-ignore no-explicit-any
		} as any;
