...
function withGithubOff(run: () => Promise<void>): Promise<void> {
    ...
    return run().finally(() => {
        if (prevMode !== undefined)
            Deno.env.set("NANO_PR_GITHUB_TRANSPORT", prevMode);
        else
            Deno.env.delete("NANO_PR_GITHUB_TRANSPORT");
        if (prevTok !== undefined)
            Deno.env.set("GITHUB_TOKEN", prevTok);
    });
}
Deno.test("re-submit of a cancelled PR clears stale open escalations + the denormalised pointer", async () => {
    await withGithubOff(async () => {
        ...
        const esc = testBoundary<Record<string, unknown>>(stores.escalations.rows[0]);
        assertEquals(esc.status, "stale");
        // … and the PR row is re-opened with the denormalised escalation pointer cleared.
        const pr = testBoundary<Record<string, unknown>>(stores.pull_requests.rows[0]);
...
Deno.test("pollIncidents mirrors an ACTIVE incident onto the PR row, then clears it, leaving status untouched", async () => {
    const row = {
        ...
        status: "converging",
        process_key: "PI-7",
        incident_key: testBoundary<string | null>(null),
        incident_message: testBoundary<string | null>(null),
        updated_at: "t0",
    };
    const stores: Record<string, {
        rows: unknown[];
    ...
    globalThis.fetch = testBoundary<typeof fetch>(incidentFetch({
    ...
    globalThis.fetch = testBoundary<typeof fetch>(incidentFetch({ "PI-7": [] }));
    try {
        await pollIncidentsImpl(data, "http://engine/v2", headers);
    }
    finally {
...
Deno.test("pollIncidents never queries a PR with no live instance and clears any stale incident", async () => {
    const noKey = {
        pr_key: "owner/repo#8",
        status: "converging",
        process_key: testBoundary<string | null>(null),
    ...
    globalThis.fetch = testBoundary<typeof fetch>((() => {
...
Deno.test("submitPr stringifies a numeric processInstanceKey (contract: string | null)", async () => {
    await withGithubOff(async () => {
        ...
        const processKey = (testBoundary(res)).processKey;
