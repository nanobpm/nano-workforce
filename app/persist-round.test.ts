...
for (const status of ["addressed", "waiting"]) {
	Deno.test(`persist-round records a '${status}' round and parks the PR in waiting_review`, async () => {
		...
		assertEquals(
			patch.status,
			"waiting_review",
			"the PR parks in waiting_review for the poller",
		);
...
Deno.test("persist-round heals a missing pull_requests parent before recording the round", async () => {
	...
	assertEquals(
		...
		"URL is derived canonically",
	);
	assertEquals(
		inserts.rounds.length,
		1,
		"the round is still recorded after the heal",
	);
	...
	// deno-lint-ignore no-explicit-any
	assertEquals(
		(updates.pull_requests![0] as any).patch.status,
		"waiting_review",
	);
});
