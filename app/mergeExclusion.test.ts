...
import { clearExclusions, deriveExclusions, type ExclusionEdge, mergeLanes, normalizePair, readExclusions, recordExclusions, } from "./mergeExclusion.ts";
...
function memData(): {
    ...
    function tbl(name: string, _pk = "id") {
...
Deno.test("deriveExclusions: an edge per file-overlapping pair, carrying the sorted overlap", () => {
    const edges = deriveExclusions(new Map([
...
Deno.test("deriveExclusions: no overlap → no edges; blank paths ignored", () => {
    assertEquals(deriveExclusions(new Map([["a", ["x.rs"]], ["b", ["y.rs"]], ["c", ["", "  "]]])), []);
});
Deno.test("recordExclusions: upserts per unordered pair — a re-scan refreshes files, never duplicates", async () => {
    const { data, stores } = memData();
    const first = await recordExclusions(data, "p", deriveExclusions(new Map([["gap-2", ["a.rs"]], ["gap-8", ["a.rs"]]])));
