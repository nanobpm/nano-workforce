...
export class WorldStore {
  ...
  async recordCheckpoint(input: RecordCheckpointInput): Promise<number> {
    ...
    const offset = await this.nextOffset(prKey);
    const now = new Date().toISOString();
    await this.#checkpoints().insert({
      pr_key: prKey,
      round_no: roundNo,
    ...
    for (const effect of effects) {
      await this.#appendEffect(prKey, offset, seq++, effect, applied, now);
    }
    return offset;
  }

  /** Append one effect to the ledger, skipping a re-record of an idempotency key already present
   * (the durable fence — the second record of one real effect is a no-op, not a second row). */
  async #appendEffect(
    prKey: string,
    offset: number,
    seq: number,
    ...
    const existing = await this.#effects().findOne({ pr_key: prKey, idempotency_key: effect.idempotencyKey });
    if (existing) return;
