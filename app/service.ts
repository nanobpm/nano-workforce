...
export const MAX_CI_FIX_ROUNDS = clampCiFixBudget(
	process.env.NANO_PR_MAX_CI_FIX_ROUNDS,
	3,
);
...
export const MAX_REBASE_ROUNDS = clampCiFixBudget(
	process.env.NANO_PR_MAX_REBASE_ROUNDS,
	3,
);
...
export const REVIEW_WAIT_TIMEOUT = reviewWaitTimeout(
	process.env.NANO_PR_REVIEW_WAIT_TIMEOUT,
);
...
export const REVIEW_NUDGE_MS =
	clampNudgeMinutes(process.env.NANO_PR_REVIEW_NUDGE_MINUTES) * 60_000;
...
const prs = (data: DataLayer) =>
	data.table<PullRequest>("pull_requests", "pr_key");
const escs = (data: DataLayer) => data.table<Escalation>("escalations", "id");
const deps = (data: DataLayer) =>
	data.table<PrDependency>("pr_dependencies", "pr_key");
...
export async function submitPr(
	...
	const depKeys = new Set(
		dependsOn.map((d) => parsePr(d)?.prKey).filter((k): k is string => !!k),
	);
	try {
	...
	if (existing) {
		...
		for (const e of await escs(data).find({
			pr_key: parsed.prKey,
			status: "open",
		})) {
...
export async function startMerge(
	...
	if (!existing?.abandon_token) {
		await prs(data).update(pr.prKey, {
			abandon_token: abandonToken,
			updated_at: now(),
		});
	}
	...
	if (processInstanceKey != null) {
		await prs(data).update(pr.prKey, {
			process_key: String(processInstanceKey),
			updated_at: now(),
		});
	}
...
export async function answerEscalation(
	...
	const open = (await escs(data).find({ pr_key: prKey, status: "open" })).sort(
		(a, b) => b.id - a.id,
	)[0];
	if (!open) return { ok: false, reason: "no open escalation" };
	const ts = now();
	await escs(data).update(open.id, {
		answer,
		status: "answered",
		answered_at: ts,
	});
			...
			const fresh = reviews
				.filter(
					(rv) =>
						rv.id > lastId &&
						rv.submitted_at &&
						(!pr.waiting_since || rv.submitted_at >= pr.waiting_since),
				)
				.sort((a, b) => a.id - b.id)
				.pop();
			if (!fresh) {
...
async function maybeRerequestReview(
	data: DataLayer,
	pr: PullRequest,
	token: string,
) {
	...
	try {
		...
		await prs(data).update(pr.pr_key, { last_nudge_at: now() });
		if (res === "requested")
			console.log(`[poller] re-requested Copilot review -> ${pr.pr_key}`);
		else if (res === "unavailable") {
			console.warn(
				`[poller] Copilot not an assignable reviewer on ${pr.pr_key}; relying on timeout`,
			);
		}
	} catch (err) {
...
async function flipToMergingThenPublish(
	...
	try {
		...
		try {
			...
			console.error(
				`[poller] revert ${prKey} -> ${prevStatus} failed: ${revertErr}`,
			);
		}
		throw err;
	}
...
async function mergeLaneDecisionForPr(
	data: DataLayer,
	prKey: string,
): Promise<PrLaneDecision | null> {
	...
	for (const lanePrKey of lanePrKeys) {
		...
		if (
			lanePr &&
			(lanePr.status === "merged" || lanePr.status === "abandoned")
		) {
			completedPrKeys.add(lanePr.pr_key);
		}
	}
	const depths = taskDependencyDepths(
		await planTaskDeps(data).find({ plan_key: planKey }),
	);
	return planPrLane(lanes, taskToPr, completedPrKeys, prKey, depths);
}

async function mirrorTaskStatusForPr(
	data: DataLayer,
	prKey: string,
	status: "opened" | "waiting-for-lane",
) {
	const ts = now();
	...
	for (const pr of await prs(data).find({ status: "waiting_merge" })) {
		...
		try {
			...
			if (verdict === "waiting") {
				...
				if (protocol) {
					const action = freshHeadRunAction(
						protocol,
						verdict,
						st.totalChecks,
						st.isDraft,
					...
					if (action) {
						const ok = await ensureFreshHeadRun(repo, number, action).catch(
							() => false,
						);
						if (ok && st.headRefOid) {
							await prs(data).update(prKey, {
								fresh_head_run_head: st.headRefOid,
								updated_at: now(),
							});
						}
						console.log(
							`[poller] fresh head run (${action}) ${ok ? "requested" : "skipped"} -> ${prKey}`,
						);
					}
				}
			...
			if (verdict === "ready") {
				...
				if (lane?.isHeld) {
					...
					console.log(
						`[poller] merge lane held by ${lane.laneHeadOf ?? "unknown"} -> ${prKey}`,
					);
					continue;
				}
			}
			...
			console.log(
				`[poller] mergeable=${verdict} (${st.mergeStateStatus}) -> ${prKey}`,
			);
		} catch (err) {
...
async function pollJobActivation(
	...
	for (const pr of all) {
		...
		if (
			worker !== (pr.active_worker ?? null) ||
			leaseUntil !== (pr.lease_until ?? null)
		) {
			await prs(data).update(pr.pr_key, {
		...
		try {
			...
			try {
				...
				try {
					...
					console.error(
						`[poller] revert wave-gate ${planKey} -> ${gateWave} failed: ${revertErr}`,
					);
				}
				throw err;
			}
