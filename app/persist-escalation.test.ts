            ...
            table(name: string, _key: string) {
                ...
                return {
                    ...
                    async insert(row: unknown) {
                        const tableInserts = inserts[name] ?? [];
                        inserts[name] = tableInserts;
                        tableInserts.push(row);
                        const pk = name === "escalations" ? "id" : name === "rounds" ? "id" : "pr_key";
                        store.set((testBoundary(row))[pk], row);
                        return name === "escalations" ? 42 : 1;
                    },
                    // deno-lint-ignore require-await
                    async update(key: string, patch: unknown) {
                        const tableUpdates = updates[name] ?? [];
                        updates[name] = tableUpdates;
                        tableUpdates.push({ key, patch });
                    },
                };
            },
        },
...
Deno.test("stalled arm (recordRound=false) does not insert a duplicate rounds row", async () => {
    ...
    const out = await handler(testBoundary(job), testBoundary(app));
    ...
    assertEquals((testBoundary(out)).escalationId, 42);
});
Deno.test("escalation arm without the flag still records the round", async () => {
    ...
    await handler(testBoundary(job), testBoundary(app));
    assertEquals(inserts.rounds.length, 1);
    assertEquals((testBoundary(inserts.rounds[0])).round_no, 3);
});
...
Deno.test("persist-escalation heals a missing pull_requests parent before recording", async () => {
    ...
    const healed = testBoundary(inserts.pull_requests[0]);
    ...
    assertEquals((testBoundary(updates.pull_requests[0])).patch.status, "escalated");
});
Deno.test("a padded question is persisted trimmed (no whitespace drift)", async () => {
    ...
    await handler(testBoundary(job), testBoundary(app));
    assertEquals((testBoundary(inserts.escalations[0])).question, "needs a decision", "escalation stores the trimmed question");
    assertEquals((testBoundary(updates.pull_requests[0])).patch.open_escalation_question, "needs a decision", "denormalised question is trimmed too");
});
...
Deno.test("blank question fabricates an answerable escalation (no throw, no incident)", async () => {
    for (const question of [undefined, "", "   "]) {
        ...
        const out = await handler(testBoundary(job), testBoundary(app));
        assertEquals((testBoundary(out)).escalationId, 42, "an escalation is opened, not refused");
        assertEquals(inserts.escalations.length, 1, "escalation row written");
        const esc = testBoundary(inserts.escalations[0]);
        assert(esc.question.trim().length > 0, "fabricated question is concrete/non-blank");
        assert(esc.question.includes("machine-readable result"), "no-result rounds explain the missing status");
        ...
        const pr = testBoundary(updates.pull_requests[0]);
...
Deno.test("persist-escalation heals from the prKey when repo/prNumber are absent", async () => {
    ...
    assertEquals(healed.abandon_token, "TOK-en_123", "the running agent's abandon token is preserved from abandonUrl, not re-minted");
});
