...
Deno.test("stalled arm (recordRound=false) does not insert a duplicate rounds row", async () => {
	...
	assertEquals(
		inserts.rounds.length,
		0,
		"no round row when the round was already recorded",
	);
...
Deno.test("persist-escalation heals a missing pull_requests parent before recording", async () => {
	...
	await handler(job as any, app as any);
	assertEquals(
		inserts.pull_requests?.length,
		1,
		"the missing parent is reconstructed",
	);
	...
	assertEquals(
		healed.status,
		"converging",
		"the healed parent starts in the converging aggregate",
	);
	...
	assertEquals(
		updates.pull_requests!.length,
		1,
		"the PR is updated once after the heal",
	);
	// deno-lint-ignore no-explicit-any
...
Deno.test("a padded question is persisted trimmed (no whitespace drift)", async () => {
	...
	// deno-lint-ignore no-explicit-any
	assertEquals(
		(inserts.escalations[0] as any).question,
		"needs a decision",
		"escalation stores the trimmed question",
	);
	// deno-lint-ignore no-explicit-any
	assertEquals(
		(updates.pull_requests![0] as any).patch.open_escalation_question,
		"needs a decision",
		"denormalised question is trimmed too",
	);
});
	...
	for (const question of [undefined, "", "   "]) {
		...
		const job = {
			variables: {
				...
				"io.nanobpm.agentResult": {
					output: "the agent's prose review, no result file",
				},
			},
		};
		// deno-lint-ignore no-explicit-any
		...
		// deno-lint-ignore no-explicit-any
		assertEquals(
			(out as any).escalationId,
			42,
			"an escalation is opened, not refused",
		);
		assertEquals(inserts.escalations.length, 1, "escalation row written");
		// deno-lint-ignore no-explicit-any
		const esc = inserts.escalations[0] as any;
		assert(
			esc.question.trim().length > 0,
			"fabricated question is concrete/non-blank",
		);
		assert(
		...
		assertEquals(
			esc.transcript,
			"the agent's prose review, no result file",
			"transcript attached",
		);
		...
		assertEquals(
			pr.patch.open_escalation_question,
			esc.question,
			"denormalised question set",
		);
	}
});
...
Deno.test("unclassified status without a question names the status in the fabricated question", async () => {
	const { app, inserts } = fakeApp();
	const job = {
		variables: { prKey: "o/r#1", round: 3, status: "in_progress" },
	};
	// deno-lint-ignore no-explicit-any
	await handler(job as any, app as any);
	// deno-lint-ignore no-explicit-any
	const esc = inserts.escalations[0] as any;
	assert(
		esc.question.includes("in_progress"),
		"fabricated question references the raw status",
	);
	assertEquals(
		esc.kind,
		"blocker",
		"a non needs_input status is a blocker escalation",
	);
});
...
Deno.test("persist-escalation heals from the prKey when repo/prNumber are absent", async () => {
	...
	assertEquals(
		inserts.pull_requests?.length,
		1,
		"the parent is reconstructed from the prKey",
	);
	// deno-lint-ignore no-explicit-any
	const healed = inserts.pull_requests![0] as any;
	...
	assertEquals(
		...
		"URL is derived from the parsed prKey",
	);
	assertEquals(
