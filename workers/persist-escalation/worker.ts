...
function fabricateQuestion(
	...
	if (!rawStatus) {
		return (
			"The review agent finished without a machine-readable result (no status was reported), " +
			"so this round could not be classified as converged, addressed, or a specific request." +
			tail
		);
	}
	return (
		`The review agent reported status "${rawStatus}" without a question, so this round ` +
...
const handler: AppJobHandler<In> = async (job, app) => {
	const { prKey, round, summary, repo, prNumber, prUrl, abandonUrl } =
		job.variables;
	...
	const question =
		nonBlank(job.variables.question) ??
		fabricateQuestion(rawStatus, transcript != null);
