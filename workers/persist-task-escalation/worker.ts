...
const handler: AppJobHandler<In, Out> = async (job, app) => {
	...
	if (!question) {
		...
		throw new Error(
			"persist-task-escalation: missing question for escalated task",
		);
	}
	...
	const existing = (
		await escTable.find({ corr_key: corrKey, status: "open" })
	...
	for (const t of await planTasks(app.data).find({
		plan_key: planKey,
		task_id: taskId,
	})) {
