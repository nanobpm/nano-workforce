...
const handler: AppJobHandler<In> = async (job, app) => {
	...
	const { prKey, repo, prNumber, prUrl, round, summary, abandonUrl } =
		job.variables;
	...
	if (AUTO_MERGE) {
		try {
			...
			if (mergeProcessKey != null) {
				...
				app.log(
					"error",
					`finalize: merge-loop start returned no process key for ${prKey}; leaving PR converged`,
				);
			}
		} catch (err) {
			app.log(
				"error",
				`finalize: could not start merge-loop for ${prKey}; leaving PR converged`,
				{
					err: String(err),
				},
			);
