...
function fakeApp(existing: PlanReview[] = []) {
	...
	const match = (r: PlanReview, q: Record<string, unknown>) =>
		Object.entries(q).every(
			([f, v]) => (r as unknown as Record<string, unknown>)[f] === v,
		);
	return {
		data: {
			table() {
				return {
					...
					findOne: (q: any) =>
						Promise.resolve(rows.find((r) => match(r, q)) ?? null),
					// deno-lint-ignore no-explicit-any
					count: (q: any) =>
						Promise.resolve(rows.filter((r) => match(r, q)).length),
...
Deno.test("unapproved, non-final round revises (planApproved=false, no throw)", async () => {
	...
	assertEquals(
		(out as { planApproved: boolean; planFindings: string }).planApproved,
		false,
	);
