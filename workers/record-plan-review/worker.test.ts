...
function fakeApp(existing: PlanReview[] = []) {
    ...
    const match = (r: PlanReview, q: Record<string, unknown>) => Object.entries(q).every(([f, v]) => (testBoundary<Record<string, unknown>>(testBoundary<unknown>(r)))[f] === v);
    return testBoundary({
        data: {
            table() {
                return {
                    findOne: (q: Record<string, unknown>) => Promise.resolve(rows.find((r) => match(r, q)) ?? null),
                    count: (q: Record<string, unknown>) => Promise.resolve(rows.filter((r) => match(r, q)).length),
...
const call = async (app: unknown, vars: Record<string, unknown>, jobKey = "j-new") => await handler(testBoundary({ variables: vars, jobKey }), testBoundary(app));
...
Deno.test("unapproved, non-final round revises (planApproved=false, no throw)", async () => {
    ...
    assertEquals((testBoundary<{
        planApproved: boolean;
        planFindings: string;
    }>(out)).planApproved, false);
    assertEquals((testBoundary<{
        planFindings: string;
    }>(out)).planFindings, "fix X");
});
Deno.test("unapproved FINAL round hard-fails with PLAN_REJECTED incident", async () => {
    ...
    const err = await assertRejects(() => call(app, { planKey: "o/r#3", approved: false, findings: "still wrong" }), BpmnError);
    assertEquals((testBoundary<BpmnError>(err)).errorCode, "PLAN_REJECTED");
});
