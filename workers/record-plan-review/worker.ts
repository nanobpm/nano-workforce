...
const handler: AppJobHandler<In, Out> = async (job, app) => {
	...
	const recorded: PlanReview =
		(await reviews.findOne({ plan_key: planKey, job_key: jobKey })) ??
		(await (async () => {
	...
	if (round + 1 >= MAX_PLAN_REVIEW_ROUNDS) {
		app.log(
			"error",
			`record-plan-review: ${planKey} not approved after ${MAX_PLAN_REVIEW_ROUNDS} round(s)`,
			{
				round,
			},
		);
	...
	app.log("info", `record-plan-review: ${planKey} round ${round} — revise`, {
		approved: false,
	});
