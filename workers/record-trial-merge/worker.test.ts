...
Deno.test("record-trial-merge shapes clean results as proceed", async () => {
    ...
    const out = testBoundary<Record<string, unknown>>(await handler(testBoundary({ key: 11, variables: { planKey: "o/r#69", currentWave: 2, result: "clean", summary: "green" } }), testBoundary(app)));
    assertEquals(out.trialMergeRed, false);
    assertEquals(inserts.plan_trial_merges.length, 1);
    assertEquals((testBoundary(inserts.plan_trial_merges[0])).result, "clean");
});
...
Deno.test("record-trial-merge treats textual conflicts as pass-through", async () => {
    const { app } = fakeApp();
    const out = testBoundary<Record<string, unknown>>(await handler(testBoundary({ variables: { planKey: "o/r#69", result: "merge-conflict", conflicts: [{ prs: [1, 2] }] } }), testBoundary(app)));
    assertEquals(out.trialMergeRed, false);
});
