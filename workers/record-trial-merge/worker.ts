...
const RESULTS = new Set<TrialMergeResult>([
	"clean",
	"merge-conflict",
	"suite-failed",
]);
...
function parseResult(v: unknown): TrialMergeResult {
	...
	return RESULTS.has(s as TrialMergeResult)
		? // biome-ignore lint/plugin: runtime/framework contract boundary for external data shape
			(s as TrialMergeResult)
		: "suite-failed";
}
...
const handler: AppJobHandler<In, Out> = async (job, app) => {
	...
	const wave = waveNo(
		job.variables.trialMergeWave ?? job.variables.currentWave,
	);
	const result = parseResult(job.variables.result);
	const summary =
		str(job.variables.summary) ??
		(result === "suite-failed"
			? "Trial merge suite failed or returned no machine-readable result"
			: result);
	// biome-ignore lint/plugin: runtime/framework contract boundary for external data shape
	...
	try {
		...
		app.log(
			"error",
			`record-trial-merge: audit persist failed for ${planKey} wave ${wave}`,
			{ err: String(err) },
		);
	}
	...
	const failing =
		Array.isArray(job.variables.failing) && job.variables.failing.length > 0
	...
	return {
		...
		summary,
		task: {
			id: trialMergeTaskId(wave),
			title: `Trial merge gate for wave ${wave}`,
		},
