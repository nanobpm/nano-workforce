...
function fakeApp(rows: Row[]) {
    ...
    const stores: Record<string, Record<string, unknown>[]> = {
        plan_tasks: testBoundary<Record<string, unknown>[]>(testBoundary<unknown>(rows)),
        plan_task_deps: [],
        pull_requests: [],
        pr_dependencies: [],
        escalations: [],
    ...
    return {
        app: testBoundary<AppApi>({
            data: {
                table(name: string, key: string) {
                    const store = stores[name] ?? [];
                    stores[name] = store;
                    return {
                        get: (k: unknown) => Promise.resolve(store.find((r) => (testBoundary<Record<string, unknown>>((testBoundary<unknown>(r))))[key] === k)),
...
function installGithubStub(method: "gh-merge" | "mergify-queue") {
    ...
    globalThis.fetch = testBoundary<typeof fetch>(((input: string | URL | Request) => {
    ...
    return () => {
        if (oldTransport == null)
            ...
            Deno.env.set("NANO_PR_GITHUB_TRANSPORT", oldTransport);
        if (oldToken == null)
            Deno.env.delete("GITHUB_TOKEN");
        else
            Deno.env.set("GITHUB_TOKEN", oldToken);
...
Deno.test("record-wave retries the same wave when a task is still pending", async () => {
    ...
    const out = testBoundary<Record<string, unknown>>(await handler(testBoundary({
        variables: {
    ...
    assertEquals((testBoundary<Record<string, unknown>>(planUpdates[0].patch)).gate_wave, 1);
});
